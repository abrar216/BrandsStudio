...
import { ArrowLeft, Upload, CheckCircle2, Video, FileText, Link as LinkIcon, Trash2 } from 'lucide-react';

export default function Edit({ auth, course, lesson }) {
    const { data, setData, post, processing, errors } = useForm({
        _method: 'PUT',
        title: lesson.title || '',
        content: lesson.content || '',
        duration: lesson.duration || '',
        order: lesson.order !== undefined ? lesson.order : 1,
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('admin.courses.lessons.update', { course: course.id, lesson: lesson.id }));
    };

    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Edit Lesson</h2>}
        >
                ...
                <div className="max-w-4xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div className="flex items-center gap-4 mb-8">
                        ...
                        <div>
                            ...
                            <p className="text-gray-500 font-medium">Updating: <span className="text-brand-accent">{lesson.title}</span></p>
                        </div>
                    </div>
                                    ...
                                    {lesson.video_path && !data.video_file && (
                                        <div className="mb-4 bg-white/5 border border-white/10 rounded-xl p-4 flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="text-white font-bold text-sm">Current Local Video</p>
                                                    <p className="text-xs text-gray-500 break-all">{lesson.video_path}</p>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                                ...
                                                <>
                                                    ...
                                                    <div className="flex text-sm text-gray-400">
                                                        <label htmlFor="video-upload" className="relative cursor-pointer rounded-md font-medium text-brand-accent hover:text-brand-accent/80 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-brand-accent">
                                                            <span>Upload a {lesson.video_path ? 'new ' : ''}video</span>
                            ...
                            <div className="space-y-6">
                                ...
                                <div>
                                    ...
                                    {lesson.pdf_path && !data.pdf_file && (
                                        <div className="mb-4 bg-white/5 border border-white/10 rounded-xl p-4 flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                <div className="bg-yellow-400/20 p-2 rounded-lg text-yellow-500">
                                                    <FileText size={20} />
                                                </div>
                                                <div>
                                                    <p className="text-white font-bold text-sm">Current PDF Resource</p>
                                                    <p className="text-xs text-gray-500 break-all">{lesson.pdf_path}</p>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                                ...
                                                <>
                                                    ...
                                                    <div className="flex text-sm text-gray-400">
                                                        <label htmlFor="pdf-upload" className="relative cursor-pointer rounded-md font-medium text-brand-accent hover:text-brand-accent/80 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-brand-accent">
                                                            <span>Upload a {lesson.pdf_path ? 'new ' : ''}PDF</span>
